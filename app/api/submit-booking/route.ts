...
export async function POST(request: Request) {
  console.log("[v0] === SUBMIT-BOOKING POST START ===");
  try {
    const body = await request.json();
    console.log("[v0] Body keys:", Object.keys(body).join(", "));
    ...
    try {
      ...
      console.log("[v0] Generated Request ID:", requestId);
    } catch (idErr) {
      console.log("[v0] ID fallback used:", requestId, idErr);
    }
    ...
    try {
      ...
      console.log("[v0] Distance:", distanceKm, "km,", durationMinutes, "min");
    } catch (distErr) {
      console.log("[v0] Distance calc failed:", distErr);
    }
    ...
    console.log("[v0] Writing row data to sheet...");
    await appendSheetRow("Bookings!A:U", [rowData]);
    console.log("[v0] Row written OK, ID:", requestId);
