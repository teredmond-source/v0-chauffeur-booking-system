...
const HEADERS = [
  ...
  "Preferred Reply", "NTA Fare Low", "NTA Fare High", "Premium Fare",
];
...
export async function POST() {
  try {
    ...
    if (!tabExisted) {
      ...
      await updateSheetRow("Bookings!A1:X1", [HEADERS]);
    ...
    let existing: string[][] = [];
    try {
      existing = await getSheetData("Bookings!A1:X1");
    } catch {
    ...
    let allData: string[][] = [];
    try {
      allData = await getSheetData("Bookings!A1:X");
    } catch {
      // Empty
    }
    ...
    await updateSheetRow(`Bookings!A1:X${newData.length}`, newData);
