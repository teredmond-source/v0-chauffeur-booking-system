        ...
        <nav className="hidden items-center gap-1 md:flex">
          {["Bookings", "Drivers", "Fleet"].map((item, i) => (
            <Link
              key={item}
              href="/"
              className={`rounded-md px-3 py-2 text-sm font-medium transition-colors hover:bg-primary-foreground/10 hover:text-primary-foreground ${
                i === 0
      ...
      <div className="space-y-3 text-sm">
        ...
        <div className="mt-3 rounded-lg border border-accent/20 bg-accent/5 p-3">
          <p className="text-xs leading-relaxed text-muted-foreground">
            Standard daytime rates. The initial charge includes the first 500m.
            All fares are NTA maximum rates for pre-booked journeys.
          </p>
        </div>
      </div>
    </div>
...
function BookingForm() {
  ...
  const handleSubmitBooking = async () => {
    ...
    try {
      ...
      setQuoteResult(null);
      toast.success(
        `Booking ${data.bookingId} created and written to Google Sheets.`
      );
    } catch (error) {
      toast.error(
    ...
    <div className="space-y-6">
      ...
      <fieldset className="space-y-3">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) =>
                updateField("pickupEircode", e.target.value.toUpperCase())
              }
        ...
        <button
          ...
          disabled={
            isQuoting ||
            !formData.pickupEircode ||
            !formData.destinationEircode
          }
      ...
      <p className="text-center text-xs text-muted-foreground">
        Booking will be calculated via Google Maps and written directly to
        Google Sheets.
      </p>
    </div>
  );
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <ul className="space-y-2 text-xs leading-relaxed text-muted-foreground">
                ...
                <li className="flex gap-2">
                  <span className="mt-0.5 h-1.5 w-1.5 shrink-0 rounded-full bg-accent" />
                  Eircodes are automatically prefixed with &quot;Ireland&quot;
                  for accurate geocoding
                </li>
                <li className="flex gap-2">
                  <span className="mt-0.5 h-1.5 w-1.5 shrink-0 rounded-full bg-accent" />
                  Use &quot;Get Fare Estimate&quot; to preview the route before
                  submitting
                </li>
                <li className="flex gap-2">
                  <span className="mt-0.5 h-1.5 w-1.5 shrink-0 rounded-full bg-accent" />
